...
class TiledPredictor:
    ...
    YOLO_CLASS_TO_MASK = {
        ...
        1: "utility_transformer_mask",  # Transformers
    }
    YOLO_LABELS = {
        0: "Well",
        1: "Transformer",
    ...
    def __init__(
        ...
        self.yolo_iou = iou_threshold = yolo_iou
    ...
    def predict_tif(
        ...
        with rasterio.open(str(tif_path)) as src:
            ...
            for x0, y0, tw_act, th_act in tqdm(windows, desc="Inference", leave=False):
                ...
                # YOLO point detection (wells / transformers)
        ...
        if roof_accum is not None:
            ...
            # Post-Process: Consolidate one roof type per building
    ...
    def predict_image(
        ...
        if roof_accum is not None:
            ...
            if "building_mask" in final_results:
                from inference.postprocess import refine_roof_types
                building_binary = (final_results["building_mask"] > 0.5).astype(np.uint8)
                roof_mask = refine_roof_types(building_binary, roof_mask)
...
def load_ensemble_pipeline(
    ...
    if loaded_ratio < 0.80:
        ...
        raise RuntimeError(
            ...
            "This often happens when checkpoint/model configs differ. "
